...
export class PIDSupportedCredentialJptVCDM implements VCDMSupportedCredentialProtocol {
	...
	getId(): string {
		return "urn:eudi:pid:1:dc:jpt";
	}
	getFormat(): VerifiableCredentialFormat {
		return VerifiableCredentialFormat.DC_JPT;
	}
	getTypes(): string[] {
	...
	async generateCredentialResponse(userSession: AuthorizationServerState, request: Request, holderPublicKeyJwk: JWK): Promise<{ format: VerifiableCredentialFormat; credential: any; }> {
		...
		const { jpt } = await this.getCredentialSigner()
			.signJptVc(
				{
	...
	exportCredentialSupportedObject(): any {
		return {
			...
			format: this.getFormat(),
			cryptographic_binding_methods_supported: ["jpt"],
			credential_signing_alg_values_supported: ["experimental/SplitBBSv2.1"],
